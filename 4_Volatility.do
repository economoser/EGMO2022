********************************************************************************
* DESCRIPTION: Compute statistics pertaining to volatility.
*
...
foreach yr of numlist $d1yrlist{
	...
	${gtools}levelsof agegp, local(agp) clean	
	foreach aa of local agp{
	
		gen float researn1F`aa' = researn1F if agegp == `aa'
		kurpercentiles "researn1F`aa'" "PKage" "`yr'"
		drop researn1F`aa' 
				
		if inlist(`yr',${d5yrlist}){
			gen float researn5F`aa' = researn5F if agegp == `aa' 
	...
	*
	...
	if inlist(`yrp',${perm3yrlist}) & inlist(`yr',${d5yrlist}){
			...
			if "${gtools}" == "" _pctile permearn, p(97.5 99 99.9)
			else gquantiles permearn, _pctile p(97.5 99 99.9)
			...
			* Calculate "kurtosis" moments within permrank
			if "`meas'" == "res"{
				gen int permrankaux = int(permrank/2.5)
				${gtools}levelsof permrankaux, local(pgp) clean	
				foreach pp of local pgp{
					gen float `meas'earn1Fp`pp' = `meas'earn1F if permrankaux == `pp'
					gen float `meas'earn5Fp`pp' = `meas'earn5F if permrankaux == `pp' 
...
// 			if "${gtools}" == "" {
				bys agegp: egen float aux1 = pctile(permearn), p(97.5)
				bys agegp: egen float aux2 = pctile(permearn), p(99)
				bys agegp: egen float aux3 = pctile(permearn), p(99.9)
// 			else {
// 				bys agegp: gegen float aux1 = pctile(permearn), p(97.5)
// 				bys agegp: gegen float aux2 = pctile(permearn), p(99)
// 				bys agegp: gegen float aux3 = pctile(permearn), p(99.9)
// 			}
			...
			if "`meas'" == "res"{
				...
				foreach pp of local pgp{				
					foreach aa of local agp {
						gen float `meas'earn1Fp`pp'_`aa' = `meas'earn1F if permrankaux == `pp' & agegp == `aa'
						gen float `meas'earn5Fp`pp'_`aa' = `meas'earn5F if permrankaux == `pp' & agegp == `aa' 
...
// 			if "${gtools}" == "" {
				bys male: egen float aux1 = pctile(permearn), p(97.5)
				bys male: egen float aux2 = pctile(permearn), p(99)
				bys male: egen float aux3 = pctile(permearn), p(99.9)
// 			else {
// 				bys male: gegen float aux1 = pctile(permearn), p(97.5)
// 				bys male: gegen float aux2 = pctile(permearn), p(99)
// 				bys male: gegen float aux3 = pctile(permearn), p(99.9)
// 			}
...
// 			if "${gtools}" == "" {
				bys male agegp: egen float aux1 = pctile(permearn), p(97.5)
				bys male agegp: egen float aux2 = pctile(permearn), p(99)
				bys male agegp: egen float aux3 = pctile(permearn), p(99.9)
// 			else {
// 				bys male agegp: gegen float aux1 = pctile(permearn), p(97.5)
// 				bys male agegp: gegen float aux2 = pctile(permearn), p(99)
// 				bys male agegp: gegen float aux3 = pctile(permearn), p(99.9)
// 			}
