********************************************************************************
* DESCRIPTION: Contains user-written programs.
*
...
program kurpercentiles
	...
	if "${gtools}" == "" _pctile `vvg', p(50)
	else gquantiles `vvg', _pctile p(50)
	gen float n`vvg' = `vvg' - r(r1)
	...
	*Share between 3sigma+
	...
	gen float sharep_m5top5 = 100*`sharep45top55'
	gen float sharep_m10top10 = 100*`sharep40top60'
	gen float sharep_m20top20 = 100*`sharep30top70'
	gen float sharep_m5tosd = 100*`sharep55tosd'
	gen float sharep_sdto2sd = 100*`sharepsdto2sd'
	gen float sharep_2sdto3sd = 100*`sharep2sdto3sd'
	gen float share_3sdp = 100*`share3sdp'
	gen float sharep_msdtom5 = 100*`sharepsdto45'
	gen float sharep_m2sdtomsd = 100*`sharep2sdtosd'
	gen float sharep_m3sdtom2sd = 100*`sharep3sdto2sd'
	gen float sharem_3sdm = 100*`share3sdm'
...
program koncentration
	...
	if "${gtools}" == "" _pctile n`vvg', p(30 40 45 50 55 60 70)		// These are to calculate -20,-10,-5,0,5,10,20
	else gquantiles n`vvg', _pctile p(30 40 45 50 55 60 70)		// These are to calculate -20,-10,-5,0,5,10,20
	...
	*Share between 3sigma+
	...
	gen float sharep45top55 = 100*`sharep45top55'
	gen float sharep40top60 = 100*`sharep40top60'
	gen float sharep30top70 = 100*`sharep30top70'
	gen float sharep55tosd = 100*`sharep55tosd'
	gen float sharepsdto2sd = 100*`sharepsdto2sd'
	gen float sharep2sdto3sd = 100*`sharep2sdto3sd'
	gen float share3sdp = 100*`share3sdp'
	gen float sharepsdto45 = 100*`sharepsdto45'
	gen float sharep2sdtosd = 100*`sharep2sdtosd'
	gen float sharep3sdto2sd = 100*`sharep3sdto2sd'
	gen float share3sdm = 100*`share3sdm'
...
program bymySho
	...
	*local suffix = "`4'"
	...
	local v2 = (r(sd))^2
	preserve
		gen float averesearn = (`varT1' + `varT2')/2
		qui: sum averesearn
		local v3 = (r(sd))^2
		
		gen float modsho = `v3'/(`v1' + `v2')
		${gtools}collapse modsho, by(`4') fast
...
program bymyTranMat
	...
	gen byte progaux = 1			// Auxiliary variable
	
	qui: drop if `varT2' ==.		// Drop if response variable is missing 
	
	${gtools}collapse (count) movecount = progaux, by(`varT1' `varT2' `suffix') fast
	
// 	if "${gtools}" == "" {
		bys `suffix' `varT1': egen long totcount = sum(movecount)
// 	}
// 	else {
// 		bys `suffix' `varT1': gegen long totcount = sum(movecount)
// 	}
	bys `suffix' `varT1':  gen float share =   100*movecount/totcount
	
	sort `suffix' `varT1' `varT2'
	order `suffix' `varT1' `varT2' movecount totcount share
	
	*Reshapes
	${gtools}reshape wide movecount totcount share, i(`suffix' `varT1') j(`varT2')
	
	*Replaces missing transitions (transitions that are not present in the data) by 0
	${gtools}levelsof `varT1', local(counts) clean
	
	*Drop totals, they are not necessary.
	drop totcount*
	
	*Reshape back to long form 
	${gtools}reshape long movecount share, i(`suffix' `varT1') j(`varT2')
...
program bymyCNTgPop
	...
	preserve
		...
		*Normalizing individual income (i.e. `varmM' is the individual share on total income)
		...
		qui: gen float aux = 100*`varM'/r(sum)	
		drop `varM'
		rename aux `varM'
		local totM = r(sum)

		sum `varMp' if (`varMp' >= rmininc[`yrpl'-${yrfirst}+1,1]) & `varMp' != .,  meanonly
		qui: gen float aux = 100*`varMp'/r(sum)	
		drop `varMp'
		rename aux `varMp'
		local totMp = r(sum)
		
		*Calculate percentile 
		if "${gtools}" == "" _pctile `varM'  if (`varM'  >= 100*rmininc[`yrl'-${yrfirst}+1,1]/`totM')  & `varM' != ., p(`qtile')		// Income in t
		else gquantiles `varM'  if (`varM'  >= 100*rmininc[`yrl'-${yrfirst}+1,1]/`totM')  & `varM' != ., _pctile p(`qtile')		// Income in t
		local top =  r(r1)
		
		if "${gtools}" == "" _pctile `varMp' if (`varMp' >= 100*rmininc[`yrpl'-${yrfirst}+1,1]/`totMp') & `varMp' != ., p(`qtile')		// Income in t+p
		else gquantiles `varMp' if (`varMp' >= 100*rmininc[`yrpl'-${yrfirst}+1,1]/`totMp') & `varMp' != ., _pctile p(`qtile')		// Income in t+p
		...
		*This is equivalent to S_t in top expression in page 16 of Gomez's paper
		...
		cap: gen byte idSt = `varM' >= `top' if `varM' != .  // This identifies individuals at the top in year t
		...
		*Top shares and identification of those at the top in period t+p
		...
		cap: gen byte idStp = `varMp' >= `topp' if `varMp' != . 	// This identifies individuals at the top in year t
		...
		*Define the displacement share (As in equation 22)
			qui: gen float aux = `varMp' - `topp'
			sum aux if (idStp == 1 & idSt == 0), meanonly	
			local RdispL = r(sum)/`St'
			drop aux
			
			qui: gen float aux = `topp' - `varMp' 
		...
		*/
		...
		gen float St = `St'
		gen float Stp = `Stp'
		gen float gSt = (`Stp' - `St')/`St' 
		gen float Rwithin = `Rwith'
		gen float Rdispla = `Rdisp'
		gen float Rretir =  `Rretir'
		gen float Rgrowth = `Rgrowth'
...
program bymyCNTg
	...
	preserve
		...
		*Calculate percentile
		if "${gtools}" == "" _pctile `varM', p(`qtile')		// Income in t
		else gquantiles `varM', _pctile p(`qtile')		// Income in t
		local top =  r(r1)
		
		if "${gtools}" == "" _pctile `varMp', p(`qtile')		// Income in t+p
		else gquantiles `varMp', _pctile p(`qtile')		// Income in t+p
		local topp =  r(r1)
		...
		*This is equivalent to S_t in top expression in page 16 of Gomez's paper
		...
		cap: gen byte idSt = `varM' >= `top' & `varM' != .  // This identifies individuals at the top in year t
		...
		*Top shares and identification of those at the top in period t+p
		...
		cap: gen byte idStp = `varMp' >= `topp' & `varMp' != . 	// This identifies individuals at the top in year t
		...
		*Decomposing the displacement share (As in equation 19)
		gen float aux = `varMp' - `topp'
		sum aux if (idStp == 1 & idSt == 0)		
		local RdispL = r(sum)/`Wt'
		drop aux
		
		gen float aux = `topp' - `varMp' 
		...
		*Define the growth in share 
		...
		gen float Rwithin = `Rwith'
		gen float Rdispla = `Rdisp'
		gen float RdispL = `RdispL'
		gen float RdispR = `RdispR'
...
program bymyRAT
	...
	preserve
		...
		if "${gtools}" == "" _pctile `varM', p(95 99.999)
		else gquantiles `varM', _pctile p(95 99.999)
		...
		gen int year = `suffix'
		forvalues ii = 0(1)`numeexp'{
			gen float tlevel`ii' = `tlevel`ii''
			gen float melevel`ii' = `melevel`ii''
			gen float ralevel`ii' = `ralevel`ii''
			gen long oblevel`ii' = `oblevel`ii''
		}
...
program bymyCNT 
	...
	*Calculate percentiles
	if "${gtools}" == "" _pctile `varM', p(20,40,50,60,80,90,95,99,99.5,99.9,99.99)
	else gquantiles `varM', _pctile p(20,40,50,60,80,90,95,99,99.5,99.9,99.99)
...
// https://www.statsdirect.com/help/default.htm#nonparametric_methods/gini.htm		
	local gini = (2.0*`ginitot' -(`numobs'+1)*`tot')/(`numobs'*`tot')		
		...
		gen float p995 = `p995'
		gen float p999 = `p999'
		gen float p9999 = `p9999'
		
		gen float q1share = `q1share'
		gen float q2share = `q2share'
		gen float q3share = `q3share'
		gen float q4share = `q4share'
		gen float q5share = `q5share'
		
		gen float bot50share = `bot50share'
		
		gen float top10share = `top10share'
		gen float top5share = `top5share'
		gen float top1share = `top1share'
		gen float top05share = `top05share'
		gen float top01share = `top01share'
		gen float top001share = `top001share'		
		
		gen float top10metop90 =  `top10metop90'
		gen float top5metop95 =   `top5metop95'
		gen float top1metop99 =   `top1metop99'
		gen float top05metop995 =  `top05metop995'
		gen float top01metop999 =  `top01metop999'
		gen float top001metop9999 = `top001metop9999'
		
		gen float top10pop = `top10pop'
		gen float top5pop = `top5pop'
		gen float top1pop = `top1pop'
		gen float top05pop = `top05pop'
		gen float top01pop = `top01pop'
		gen float top001pop = `top001pop'
...
program bymyPCT 
	...
	if "${gtools}" == "" {
		...
		qui statsby $statlist1 $statlist2 $statlist3 $statlist4, by(`4') saving(PC_`prefix'`varM'`suffix',replace): ///
		gquantiles `varM', _pctile p(1,2.5,5,10,12.5,25,37.5,50,62.5,75,87.5,90,95,97.5,99,99.9,99.99)
	}
...
program bymypctile, byable(recall)
	...
	marksample touse 
	tempvar temp_rank
	if "${gtools}" == "" capture noisily _pctile `1' if `touse', p(`numq')
	else capture noisily gquantiles `1' if `touse', _pctile p(`numq')
		forvalues hh = 1/`4'  {
