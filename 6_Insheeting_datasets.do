********************************************************************************
* DESCRIPTION: Generate data files to be uploaded to GIDDP website.
*
...
// 	global maindir ="..."		// Define main directory
	...
	global ineqdata = "09 Feb 2022 Inequality"			// Data on Inequality 
	global voladata = "09 Feb 2022 Volatility"			// Data on Volatility
	global mobidata = "09 Feb 2022 Mobility"			// Data on Mobility

	global datafran="${folder}${sep}26 Feb 2022 Upload"			// Define were data will be saved
	...
	global logfile = "26 Feb 2022"
	global logfile="$logfile Insheeting_datasets"
...
// 	drop if year < 1950
	...
	*Save
	...
	gen i = 1
	${gtools}reshape long slopep_tp5 slopep_tp1, i(i) j(year)	
	g str3 country="${iso}"
	g str12 age="25-55"
	if `mm' == 1{		
		g str12 gender="Male"
...
${gtools}reshape long val_logearn den_logearn,i(index) j(year)
g str3 country="${iso}"
...
preserve
	...
	${gtools}collapse (sum) n${varx} (mean) mean${varx}-p99_99${varx},by(groupage gender country year) fast
...
foreach ff in 1 5{
		...
		${gtools}reshape long val_`x'earn`ff'f den_`x'earn`ff'f,i(index) j(year)
		g str3 country="${iso}"
...
preserve
	...
	replace groupage=2 if age>=35 & age<=44
	replace groupage=3 if age>=45 & age<=55
	${gtools}collapse (sum) n`x'earn`ff' (mean) mean`x'earn`ff'-p99_99`x'earn`ff',by(groupage gender country year) fast
...
${gtools}collapse *earn*f,by(permrank) fast
	g str3 country="${iso}"
