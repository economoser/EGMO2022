********************************************************************************
* DESCRIPTION: Compute statistics pertaining to mobility.
*
...
if $pme_munis global numestates = "1 3 5 10"		// How many years ahead mobility
else global numestates = "1 3 5 10 15"		// How many years ahead mobility

global mathetgroup = `" "male agegp" "male" "agegp" "all" "' // Mobility conditional on
	...
	foreach subgp in $numestates{		// This is the of years that will be used for the jump
		...
		forvalues yr = `firstyr'/`yrmax'{		
			...
			*Load the data							
			...
			"${TEMP_DIR}${sep}dta${sep}master_sample.dta" if `varx'`yr'~=. & `varx'`yrp'~=. , clear 
...
// 				else {
// 					by `het' : gegen numobs=count(`varx'`yr`t'')
// 				}
...
foreach varx in permearnalt researn{
	...
	foreach het in "all" "male" "agegp" "male_agegp"{ // For each heterogeneity group
		...
		foreach subgp in $numestates{ // Number of years that will be used for the jump		
			clear
			local yrmax = ${yrlast} - `subgp' 	// This should not be negative
// 			disp "local varx = `varx', local het = `het', local subgp = `subgp', global yrlast = ${yrlast}, local firstyr = `firstyr', local yrmax = `yrmax'" // debugging by Chris for RAIS-PME on January 29, 2021
